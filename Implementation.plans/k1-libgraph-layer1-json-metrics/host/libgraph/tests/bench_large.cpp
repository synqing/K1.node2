...
int main(int argc, char** argv) {
    ...
    if (!jsonPath.empty()) {
        try {
            g = k1::graph::io::load_csr_from_json_file(jsonPath);
        } catch (const std::exception& e) {
            std::cerr << "Failed to load JSON graph: " << e.what() << "\n";
            return 3;
        }
        sourceDesc = std::string("json:") + jsonPath;
    } else {
        if (has_flag(args, "--layers")) {
            std::istringstream(arg_or(args, "--layers", "200")) >> layers;
        }
        if (has_flag(args, "--width")) {
            std::istringstream(arg_or(args, "--width", "500")) >> width;
        }
        g = make_layered_dag(layers, width, true);
        std::ostringstream ss; ss << "generated:layers=" << layers << ",width=" << width;
        sourceDesc = ss.str();
    }
    ...
    if (!k1::graph::io::save_bench_topo_json(g, ms, outBench, sourceDesc)) {
        std::cerr << "Warning: could not write " << outBench << "\n";
    }
    auto metrics = k1::graph::io::compute_metrics(g);
    if (!k1::graph::io::save_graph_metrics_json(metrics, outMetrics)) {
        std::cerr << "Warning: could not write " << outMetrics << "\n";
    }

    // Human-readable memory approximation

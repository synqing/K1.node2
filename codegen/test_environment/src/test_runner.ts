...
import { validateGraph, generateValidationReport, ValidationResult } from './validation_tests.js';
 ...
 */
function validateGraphDirectory(dirPath: string): TestSummary {
 ...
 */
function generateComprehensiveReport(summary: TestSummary): string {
    ...
    const criticalViolations = summary.results.filter(r => 
        r.result.violations.some(v => v.severity === 'critical'));
    const majorViolations = summary.results.filter(r => 
        !r.passed && !r.result.violations.some(v => v.severity === 'critical'));
...
program
    ...
    .action((inputDir: string, options: { output?: string }) => {
        try {

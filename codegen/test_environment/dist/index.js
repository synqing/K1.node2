...
`;
function generatePaletteData(paletteData) {
...
function generatePaletteInterpolation(paletteData) {
    if (!paletteData || paletteData.length === 0) {
        return `
        color_buffer[i].r = 0;
        color_buffer[i].g = 0;
...
function generateNodeCode(node, graph) {
    switch (node.type) {
        ...
        case 'palette_interpolate': {
            const palette = node.parameters?.palette || 'default';
            ...
            const paletteColors = graph.palette_data.map((entry) => {
                let r, g, b;
    ...
    for (int i = 0; i < NUM_LEDS; i++) {
        ...
        case 'sin': {
            ...
            const inputNode = graph.nodes.find(n => n.id === inputNodeId);
            if (!inputNode)
                throw new Error(`sin input node not found: ${inputNodeId}`);
            const inputCode = generateNodeCode(inputNode, graph);
        ...
        case 'clamp': {
            ...
            const inputNode = graph.nodes.find(n => n.id === inputNodeId);
            if (!inputNode)
                throw new Error(`clamp input node not found: ${inputNodeId}`);
            const inputCode = generateNodeCode(inputNode, graph);
        ...
        case 'modulo': {
            ...
            const inputNode = graph.nodes.find(n => n.id === inputNodeId);
            if (!inputNode)
                throw new Error(`modulo input node not found: ${inputNodeId}`);
            const inputCode = generateNodeCode(inputNode, graph);
        ...
        case 'scale': {
            ...
            const inputNode = graph.nodes.find(n => n.id === inputNodeId);
            if (!inputNode)
                throw new Error(`scale input node not found: ${inputNodeId}`);
            const inputCode = generateNodeCode(inputNode, graph);
        ...
        case 'spectrum_range': {
            ...
            const band = node.parameters?.band || 'custom';
            if (band === 'low') {
...
function validateCenterOriginCompliance(graph) {
    for (const node of graph.nodes) {
        ...
        if (node.type === 'gradient') {
            throw new Error(`Center-origin violation: Node "${node.id}" uses forbidden gradient type.\n` +
        ...
        if (node.type === 'hsv_to_rgb') {
            console.warn(`Warning: Node "${node.id}" uses hsv_to_rgb (legacy rainbow converter).\n` +
...
function isAudioReactive(graph) {
...
function compileMultiPattern(graphs) {
    ...
    if (audioReactivePatterns.length > 0 && audioStartCount === 0) {
        throw new Error('CRITICAL ERROR: Audio-reactive patterns detected but PATTERN_AUDIO_START macro missing!\n' +
    ...
    if (audioReactivePatterns.length !== audioStartCount) {
        console.warn(`⚠️  WARNING: Macro count mismatch!\n` +
...
function loadGraphsFromDirectory(dirPath) {

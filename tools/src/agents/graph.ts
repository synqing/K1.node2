...
const edges: number[] = csr.edges;
const offsets: number[] = csr.offsets;
...
function topoDag(): { order: number[]; isDag: boolean } {
  const indeg = Array(N).fill(0);
  for (let u=0; u<N; u++) for (let e=offsets[u]; e<offsets[u+1]; e++) indeg[edges[e]]++;
  const q: number[] = [];
  for (let i=0;i<N;i++) if (indeg[i]===0) q.push(i);
  const order: number[] = [];
  for (let i=0;i<q.length;i++) {
    const u=q[i]; order.push(u);
    for (let e=offsets[u]; e<offsets[u+1]; e++) { const v=edges[e]; if (--indeg[v]===0) q.push(v); }
  }
